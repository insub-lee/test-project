  ...
  render() {
    ...
    const renderButton = () => {
      ...
      if (toggleChildrenVisibility) {
        if ((node.children && (node.children.length > 0 || typeof node.children === 'function')) || node.IS_FOLDER === 'true') {
          jsx = (
            <div>
              <button
          ...
          jsx = (
            <div>
              <button
                ...
                className={`${styles.ordinaryButton} rstcustom__ordinaryButton ${selectedIndex === node.key ? 'active' : `${selectedIndex}/${node.key}`}`}
                style={{ left: -0.5 * scaffoldBlockPxWidth }}
              />
            </div>
          );
