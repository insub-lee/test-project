...
function* saveNode({ nodeInfo }) {
  const response = yield call(Axios.post, `/api/workflow/v1/common/node/-1`, { PARAM: { ...nodeInfo } });
  const { code } = response;
  if (code !== undefined && code === 200) {
    message.success(<MessageContent>노드 등록에 성공하였습니다.</MessageContent>, 3);
    yield put(push('/admin/adminmain/node'));
  } else {
    feed.error(`노드 등록에 실패하였습니다.`);
  }
}
