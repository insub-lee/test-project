import { defineMessages } from 'react-intl';

export default defineMessages({
  skPortalTitle: {
    id: 'portal.App.components.Header.skPortalTitle',
    defaultMessage: '차세대반응형포털',
  },
  skManagement: {
    id: 'portal.App.components.Header.skManagement',
    defaultMessage: '경영',
  },
  skMediaCenter: {
    id: 'portal.App.components.Header.skMediaCenter',
    defaultMessage: '미디어센터',
  },
  skCommunity: {
    id: 'portal.App.components.Header.skCommunity',
    defaultMessage: '커뮤니티',
  },
  skLife: {
    id: 'portal.App.components.Header.skLife',
    defaultMessage: '라이프',
  },
  linkToTokTok: {
    id: 'portal.App.components.Header.linkToTokTok',
    defaultMessage: 'tokotk 가기',
  },
  skEthicalManagementReport: {
    id: 'portal.App.components.Header.skEthicalManagementReport',
    defaultMessage: '윤리경영제보',
  },
  linkToBizStore: {
    id: 'portal.App.components.Header.linkToBizStore',
    defaultMessage: '비즈스토어 가기',
  },
  skHynix: {
    id: 'portal.App.components.Header.skHynix',
    defaultMessage: 'Company',
  },
  knowledgeSearch: {
    id: 'portal.App.components.Header.knowledgeSearch',
    defaultMessage: '지식검색',
  },
  search: {
    id: 'portal.App.components.Header.search',
    defaultMessage: '검색하기',
  },
  emailSite: {
    id: 'portal.App.components.Header.emailSite',
    defaultMessage: '이메일 사이트',
  },
  yellowPagePopup: {
    id: 'portal.App.components.Header.yellowPagePopup',
    defaultMessage: '옐로페이지(팝업창)',
  },
  certificationCenter: {
    id: 'portal.App.components.Header.certificationCenter',
    defaultMessage: '인증센터',
  },
  groupMobilePortal: {
    id: 'portal.App.components.Header.groupMobilePortal',
    defaultMessage: '그룹모바일포탈',
  },
  toktokGuide: {
    id: 'portal.App.components.Header.toktokGuide',
    defaultMessage: 'toktok가이드',
  },
  selectLanguage: {
    id: 'portal.App.components.Header.selectLanguage',
    defaultMessage: '언어선택',
  },
  logout: {
    id: 'portal.App.components.Header.logout',
    defaultMessage: '로그아웃',
  },
  home: {
    id: 'portal.App.components.Header.home',
    defaultMessage: 'Home',
  },
});
