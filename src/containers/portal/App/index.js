...
// import { RenderCheckComponent } from 'utils/RenderCheckComponent';
...
class App extends React.PureComponent {
  ...
  isEqualDockAppList = (nextProps) => {
    const { dockAppList } = this.props;

    if (dockAppList.length !== nextProps.dockAppList.length) {
      // 추가 혹은 삭제
      return false;
    }
    if (_.isEqualWith(dockAppList, nextProps.dockAppList)) {
      // 그대로
      return true;
    }
          ...
          >
            <AppWrapper style={{ width: '100%' }}>
              ...
              >
                <div
                  id="child"
                  className={setMyMenuData.APP_YN === 'Y' || setMyMenuData.INTL_TYPE === 'Y' ? '' : 'gridWrapper'}
                >
                  <Content
...
App.propTypes = {
  ...
  dataForApps: PropTypes.object.isRequired,
...
const mapDispatchToProps = dispatch => ({
  ...
  // RoutesAction의 디스패쳐
  handleLoadSkin: () =>
    dispatch(routesAction.loadSkin()),
  handleDockSetMyMenuData: (pageId, isHome) =>
    dispatch(routesAction.dockSetMyMenuData(pageId, isHome)),
  handleExitDockItem: dockId =>
    dispatch(routesAction.exitDockItem(dockId)),
  handleClickApps: node =>
    dispatch(routesAction.execApps(node)),
  handleFixDockItem: dockId =>
    dispatch(routesAction.fixDockItem(dockId)),
  handleUnfixDockItem: dockId =>
    dispatch(routesAction.unfixDockItem(dockId)),
  handleDndChangePosition: (appDockId, afterDockId) =>
    dispatch(routesAction.dndChangePosition(appDockId, afterDockId)),
  handleDndChangePositionSaga: () =>
    dispatch(routesAction.dndChangePositionSaga()),
  handleSetIsUnfixDockItem: () =>
    dispatch(routesAction.setIsUnfixDockItem()),
  handleExecMenu: (PAGE_ID, TARGET, node) =>
    dispatch(routesAction.execMenu(PAGE_ID, TARGET, node)),
  ...
  handleSetDockFixedYn: dockFixedYn =>
    dispatch(routesAction.setDockFixedYn(dockFixedYn)),
  handleSetDockIconType: dockIconType =>
    dispatch(routesAction.setDockIconType(dockIconType)),
  handleReload: item =>
    dispatch(routesAction.reload(item)),
  handleLoadBoard: num =>
    dispatch(boardAction.getIfDetailBoardList(num)),
  handleGetDataForApps: EXEC_PAGE_IDS =>
    dispatch(routesAction.getDataForApps(EXEC_PAGE_IDS)),
  handleSaveApps: (apps, setMyMenuData) =>
    dispatch(routesAction.saveApps(apps, setMyMenuData)),
});
