...
class BizMenuAuthSetting extends Component {
  ...
  componentDidUpdate() {
    const { match, history, dataP } = this.props;
    const { params } = match;
    const { BIZGRP_ID, MENU_ID } = params;
...
const withReducer = injectReducer({ key: 'userStore/AppMain/BizManage/BizMenuReg/BizMenuAuthSetting', reducer });
const withSaga = injectSaga({ key: 'userStore/AppMain/BizManage/BizMenuReg/BizMenuAuthSetting', saga });
