...
class AppSec extends Component {
  ...
  setColumnsAndRowGetter = () => {
    this.setState({
      columns: [
        {
          key: 'CHECKBOX',
          name: <Checkbox checked={this.state.checkedAll} onChange={this.gridHeaderCheckboxOnChange} />,
          width: 60,
  ...
  gridHeaderCheckboxOnChange = () => {
    const { checkedAll, rowGetter } = this.state;
  ...
  confirmOnClick = loadingSet => {
    ...
    const { handleConfirmRequest } = this.props;
  ...
  SetCheckbox = val => {
    ...
    if (reqStatusCd === 'P') {
      return <Checkbox checked={val.dependentValues.check} onChange={this.onChecked} value={val.dependentValues} />;
    } else {
      return <Checkbox checked={val.dependentValues.check} onChange={this.onChecked} value={val.dependentValues} disabled={true} />;
    }
    return '';
  };
      ...
      content={
        <div>
          <ul className="userProfileMenuList">
            <li>
              <Button
                onClick={() => this.handleClickToMoveToSite({ USER_ID: val.dependentValues.USER_ID }, 'org')}
                type="button"
                className="highlight icon-info"
              >
            ...
            <li>
              <Button onClick={() => this.handleClickToMoveToSite(val.dependentValues, 'mail')} type="button" className="icon-mail">
                {intlObj.get(messages.sendToMail)}
              </Button>
            </li>
            <li>
              <Button onClick={() => this.handleClickToMoveToSite(val.dependentValues, 'todo')} type="button" className="icon-todo">
    ...
    if (val.dependentValues.REQ_STATUS_CD === 'C') {
      return (
        <span
          ...
          onKeyUp={e => {
            if (e.keyCode === 13) this.onOppose('C', val.dependentValues);
          }}
          role="button"
          tabIndex="0"
        />
      ...
      content={
        <div>
          <ul className="userProfileMenuList">
            <li>
              <Button
                onClick={() => this.handleClickToMoveToSite({ USER_ID: val.dependentValues.CONF_USER_USER_ID }, 'org')}
                type="button"
                className="highlight icon-info"
              >
                {intlObj.get(messages.userProfile)}
            ...
            <li>
              <Button onClick={() => this.handleClickToMoveToSite({ EMAIL: val.dependentValues.CONF_USER_EAMIL }, 'mail')} type="button" className="icon-mail">
                {intlObj.get(messages.sendToMail)}
              </Button>
            </li>
            <li>
              <Button onClick={() => this.handleClickToMoveToSite({ EMP_NO: val.dependentValue.CONF_USER_EMP_NO }, 'todo')} type="button" className="icon-todo">
                {intlObj.get(messages.todoRegist)}
              </Button>
            </li>
  ...
  SetReason = val => {
    return (
      <Popover
        ...
        content={
          <div>
            {val.dependentValues.REQ_STATUS_CD === 'P'
              ? intlObj.get(messages.requestReason)
              : val.dependentValues.REQ_STATUS_CD === 'R'
              ? intlObj.get(messages.returnReason)
              : intlObj.get(messages.confirmReason)}
            &nbsp;:&nbsp;
  ...
  render() {
    const { appSecList, appAuthCnl, handleGetAppSecList, handleReturnRequest, handleCancelRequest } = this.props;
    const {
      selectedAppCancel,
      SORT_COLUMN,
      SORT_DIRECTION,
    ...
    const handleGridSort = (sortColumn, sortDirection) => {
      this.setState(
        ...
        () => {
          handleGetAppSecList(PAGE, PAGE_CNT, sortColumn, sortDirection, APP_ID, REQ_STATUS_CD, SEARCH_TEXT);
        },
      );
    };
    ...
    const handleKeyPress = val => {
      if (val.key === 'Enter') {
        PAGE = 1;
        handleGetAppSecList(PAGE, PAGE_CNT, SORT_COLUMN, SORT_DIRECTION, APP_ID, REQ_STATUS_CD, SEARCH_TEXT);
      }
    };
    ...
    return (
      <div>
        <Organization show={organizationShow} closeModal={this.closeModal} isModal={true} isProfile={true} userProfile={userProfile} />
        <StyleApp>
          <div className="searchBox">
            <Select defaultValue="A" style={{ width: 130 }} onChange={selectboxStatusOnChange}>
            ...
            <div className="searchWrapper">
              ...
              />
              <button title={intlObj.get(messages.search)} className="searchBtn" onClick={searchOnClick} />
            </div>
          </div>
          <StyleDataGrid>
            <ReactDataGrid
              columns={this.state.columns ? this.state.columns : []}
              rowGetter={this.state.rowGetter ? this.state.rowGetter : () => appSecList[0]}
          ...
          <div className="buttonWrapper">
            ...
            <BtnDkGray
              onClick={() => {
                this.confirmOnClick(loadingSet);
              }}
            >
...
export function mapDispatchToProps(dispatch) {
  return {
    handleGetAppSecList: (page, pageCnt, SORT_COLUMN, SORT_DIRECTION, APP_ID, REQ_STATUS_CD, SEARCH_TEXT) =>
      dispatch(actions.getAppSecList(page, pageCnt, SORT_COLUMN, SORT_DIRECTION, APP_ID, REQ_STATUS_CD, SEARCH_TEXT)),
    handleReturnRequest: (REQ_ID_ARR, COMMENT, loadingSet) => dispatch(actions.returnRequest(REQ_ID_ARR, COMMENT, loadingSet)),
    handleCancelRequest: (REQ_ID, COMMENT, loadingSet) => dispatch(actions.cancelRequest(REQ_ID, COMMENT, loadingSet)),
    handleConfirmRequest: (REQ_ID_ARR, loadingSet) => dispatch(actions.confirmRequest(REQ_ID_ARR, loadingSet)),
  };
}
