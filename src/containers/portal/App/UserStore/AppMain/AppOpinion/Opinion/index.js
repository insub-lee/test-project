...
class AppOpinion extends Component {
  ...
  setApps = val => {
    ...
    return (
      <div className="apps">
        <Link to={{ pathname: '/portal/store/appMain/AppOpinion/ExaminePage', state: { status: value, oppoList: this.props.oppoList } }} title={val.value}>
          {val.value}
        </Link>
      </div>
    );
  ...
  SetCheckbox = val => {
    ...
    if (check === 'P') {
      return (
        <div>
          <div className="contents">
            <Checkbox onChange={this.onChecked} value={val.dependentValues} checked={val.dependentValues.check ? true : false} />
          </div>
        </div>
      );
    } else {
