...
export function getfullPathFromSelectedIndex({ treeData, selectedIndex }) {
  let fullpath = [];
  walk({
    treeData,
    ...
    callback: nodeInfo => {
      const { path } = nodeInfo;
      if (path[path.length - 1] === selectedIndex) {
        fullpath = path;
      }
      return true;
    },
...
export function toggleExpandedForSelected({ treeData, selectedIndex }) {
  let fullpath = [];
  walk({
    treeData,
    getNodeKey: ({ node }) => node.key,
 ...
 * @param {function|any} newNode - Node to replace the node at the path with, or a
 * function producing the new node
 * @param {!function} getNodeKey - Function to get the key from the nodeData and tree index
 * @param {boolean=} ignoreCollapsed - Ignore children of nodes without `expanded` set to `true`
 *
 * @return {Object[]} changedTreeData - The changed tree data
 ...
 * @return {number} result.treeIndex - The tree index at which the node was inserted
 * @return {number[]|string[]} result.path - Array of keys leading to the node location
 * after insertion
...
export function getFlatDataFromTree({ treeData, getNodeKey, ignoreCollapsed = true }) {
  ...
  walk({
    ...
    getNodeKey,
    ignoreCollapsed,
    callback: nodeInfo => flattened.push(nodeInfo),
  });

  return flattened;
}

export function getFlatMapDataFromTree({ treeData, getNodeKey, ignoreCollapsed = true }) {
  // if (!treeData || treeData.length < 1) {
  //   return [];
  // }

  let flattened = fromJS({});
  walk({
    treeData,
    getNodeKey,
    ignoreCollapsed,
    callback: nodeInfo => {
      const { node } = nodeInfo;
      flattened = flattened.set(node.key, node);
      return true;
    },
  });
 ...
 * @param {string|number=} rootKey - The value returned by `getParentKey`
 * that corresponds to the root node.
 *                                  For example,
 * if your nodes have id 1-99, you might use rootKey = 0
 *
 * @return {Object[]} treeData - The flat data represented as a tree
 */
export function getTreeFromFlatData({ flatData, getKey = node => node.key, getParentKey = node => node.PRNT_ID, rootKey }) {
  if (!flatData) {
    return [];
  }
 ...
 * @param {!function} getNodeKey - Function to get the key from the nodeData and tree index
 * @param {!Object[]} treeData - Tree data
 * @param {?string|number} searchQuery - Function returning a boolean to indicate whether
 * the node is a match or not
 * @param {!function} searchMethod - Function returning a boolean to indicate whether th
 *  node is a match or not
 * @param {?number} searchFocusOffset - The offset of the match to focus on
 *                                      (e.g., 0 focuses on the first match, 1 on the second)
 * @param {boolean=} expandAllMatchPaths - If true, expands the paths to any matched node
 * @param {boolean=} expandFocusMatchPaths - If true, expands the path to the focused node
 *
 * @return {Object[]} matches - An array of objects containing the matching `node`s, their
 *  `path`s and `treeIndex`s
