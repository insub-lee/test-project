...
const selectAppDetail = state => state.get('appRating');

const makeSelectAppRatingInfo = () => createSelector(
  selectAppDetail,
  appRatingState => appRatingState.get('appRatingInfo'),
);

const makeSelectAppRatingList = () => createSelector(
  selectAppDetail,
  appRatingState => appRatingState.get('appRatingList').toJS(),
);

const makeSelectMyAppRating = () => createSelector(
  selectAppDetail,
  appRatingState => appRatingState.get('myAppRating'),
);

const makeSelectRtheiFlog = () => createSelector(
  selectAppDetail,
  appRatingState => appRatingState.get('rtheiFlog'),
);

const makeSelectAppRatingListSize = () => createSelector(
  selectAppDetail,
