...
class Organization extends Component {
  ...
  getTab = () => {
    ...
    if (tabType[content.length] === 'attendGrp') {
      /* eslint-disable */
      content.push(
        <div className="attendGrp" key={content.length}>
          <List
            attendGroupList={this.props.attendGroupList}
            handleGetAttendMemberList={handleGetAttendMemberList}
          />
          <div className={isTab === false ? 'userGridResult userGridResultNoTab' : 'userGridResult'}>
            <div className="userSearch">
              <div className="inputWrapper" ref={ref => this.searchInputUser = ref}>
                    ...
                    <Col xl={16} style={colStyle} className="leftActivity">
                      {
                        isTab ?
                          <Tabs selected={this.state.selected} onSelect={this.handleSelect}>
                            {content.map((v, i) => {
                              const e = React.createElement;
                              return e(Tab, { label: intlObj.get(messages[tabType[i] === 'user' ? 'member' : 'attendGroup']) }, v);
                            })}
                          </Tabs>
                          :
                          <div className='noTab'>
...
Organization.propTypes = {
  ...
  handleGetAttendGroupList: PropTypes.func.isRequired,
  handleGetAttendMemberList: PropTypes.func.isRequired,
  attendGroupList: PropTypes.array.isRequired,
  attendMemberList: PropTypes.array.isRequired,
};
...
export function mapDispatchToProps(dispatch) {
  return {
    ...
    handleGetAttendGroupList: () => dispatch(actions.getAttendGroupList()),
    handleGetAttendMemberList: GRP_ID => dispatch(actions.getAttendMemberList(GRP_ID)),
  };
}
const mapStateToProps = createStructuredSelector({
  ...
  attendGroupList: selectors.makeSelectAttendGroupList(),
  attendMemberList: selectors.makeSelectAttendMemberList(),
});
const withConnect = connect(mapStateToProps, mapDispatchToProps);
const withReducer = injectReducer({ key: 'bookroomOrg', reducer });
