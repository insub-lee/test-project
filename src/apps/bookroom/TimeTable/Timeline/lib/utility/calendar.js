...
export function getMinUnit(zoom, width, timeSteps) {
  ...
  Object.keys(timeDividers).some(unit => {
    ...
    const cellWidthToUse =
      //timeSteps[unit] && timeSteps[unit] > 1 ? 3 * minCellWidth : minCellWidth    헤더를 항상 시간대로 표기하기 위해 배율 조정 3==>1  kkj
      timeSteps[unit] && timeSteps[unit] > 1 ? 1 * minCellWidth : minCellWidth 
