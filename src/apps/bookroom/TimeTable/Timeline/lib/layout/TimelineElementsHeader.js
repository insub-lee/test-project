...
export default class TimelineElementsHeader extends Component {
  ...
  subHeaderLabel(time, unit, width) {
    ...
    if (unit === 'year') {
      ...
      return time.format(f.hourLong) // kkj 무조건 hourMedium return time.format(width < 50 ? f.hourMedium : f.hourMedium)
    } else if (unit === 'minute') {
        ...
        (time, nextTime) => {
          ...
          topHeaderLabels.push(
            <div
              ...
              // onClick={() => this.handlePeriodClick(time, nextUnit)} kkj 이벤트 방지
              style={{
                left: `${left - 1}px`,
                width: `${labelWidth}px`,
            ...
            >
              <span style={{ width: contentWidth, display: 'block' }}>
                { /* kkj 나오지 않도록 변경 this.headerLabel(time, nextUnit, labelWidth) */ }
              </span>
            </div>
          )
        }
      ...
      (time, nextTime) => {
        ...
        bottomHeaderLabels.push(
          <div
            ...
            // onClick={() => this.handlePeriodClick(time, minUnit)} kkj 이벤트 방지
            style={{
              ...
              fontSize: `${
                // labelWidth > 30 ? '10' : labelWidth > 20 ? '10' : '10' // kkj 시간 폰트 사이즈
                (headerLabelGroupHeight + headerLabelHeight >= 20 && labelWidth) > 20 ? '10' : '0'
              }px`,
              cursor: 'pointer'
            }}
          >
