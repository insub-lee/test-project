...
export default class Item extends Component {
  ...
  onTouchEnd = e => {
    if (!this.state.interactMounted && this.startedTouching) {
      /* kkj 아이템 터치 후 스크롤시 상세 화면 열리지 않도록 수정 */
  ...
  getItemStyle(props) {
    ...
    const baseStyles = {
      ...
      left: `${dimensions.left+2}px`, // kkj `${dimensions.left}px`,
      top: `${dimensions.top}px`,
      width: `${dimensions.width-4}px`, // kkj `${dimensions.width}px`,
