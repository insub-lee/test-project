...
class ChemicalManagePage extends Component {
  ...
  componentDidMount() {
    ...
    const { SITE: site, GONGNO: gongno, PRODNM: prodnm, GONGAREA: gongarea, GONGINFO: gonginfo } = initData;
    const apiInfo = {
      key: 'getChemFabList',
      type: 'GET',
      url: `/api/gcs/v1/common/chemiacal/manage?type=fabList&site=${site}&gongno=${gongno}&prodnm=${prodnm}&gongarea=${gongarea}&gonginfo=${gonginfo}`,
    };
  ...
  submitFormData = apiType => {
    ...
    const submitData = {
      PARAM: {
        ...
        TYPE: 'fabList',
        SITE: initData.SITE,
        GONGNO: initData.GONGNO,
        PRODNM: initData.PRODNM,
        GONGAREA: initData.GONGAREA,
        GONGINFO: initData.GONGINFO,
      },
    };
    switch (apiType) {
      case 'ADD':
        submitHandlerBySaga(id, 'POST', '/api/gcs/v1/common/chemiacal/manage', submitData, this.addCollback);
        break;
      case 'MODIFY':
        submitHandlerBySaga(id, 'PUT', '/api/gcs/v1/common/chemiacal/manage', submitData, this.updateCollback);
        break;
      case 'DELETE':
        submitHandlerBySaga(id, 'DELETE', '/api/gcs/v1/common/chemiacal/manage', submitData, this.deleteCollback);
        break;
      default:
        break;
    }
