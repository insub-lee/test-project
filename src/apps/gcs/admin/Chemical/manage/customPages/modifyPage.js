...
import UseInfoViewTable from '../infoTable/useInfoViewTable';
...
class ModifyPage extends Component {
  ...
  componentDidMount() {
    const { sagaKey: id, getProcessRuleByModify, workInfo, workPrcProps, draftInfo, tempProcessRule, setProcessRule, relType, setRelType, initData, changeFormData, formData } = this.props;
    ...
    // 수정상태 진입시, 월사용수량, 월사용량, 연사용, 연사용수량 업데이트 (asis에 구현은안되어 있으나 혹시나 해서 만들어 둠)
    // if (formData) {
    //   const { CHEMUSAGEY, CHEMDISCHARGEY } = initData;
    //   const month = moment(formData.CHEMMDT, 'YYYYMM').format('MM');
    //   const targetMonth = `MONTH${Number(month)}`;
    //   const targetMonthUse = `MONTH${Number(month)}_USE`;
    //   if (initData[targetMonth] !== 0) changeFormData(id, 'CHEMDISCHARGEM', initData[targetMonth]);
    //   if (initData[targetMonthUse] !== 0) changeFormData(id, 'CHEMUSAGEM', initData[targetMonth]);
    //   if (CHEMUSAGEY !== 0) changeFormData(id, 'CHEMUSAGEY', CHEMUSAGEY);
    //   if (CHEMDISCHARGEY !== 0) changeFormData(id, 'CHEMDISCHARGEY', CHEMDISCHARGEY);
    // }
  }
        ...
        <StyledWrap className={viewPageData.viewType}>
          <Sketch {...bodyStyle}>
            ...
            <div style={{ marginTop: '20px' }}>
              <UseInfoViewTable formData={initData} />
            </div>
            <StyledButtonWrapper className="btn-wrap-center btn-wrap-mt-20">
              ...
              <Popconfirm
                ...
                onConfirm={() => deleteTask(id, reloadId, viewPageData.workSeq, viewPageData.taskSeq, this.changeTaskCallback)}
                okText="Yes"
                cancelText="No"
              >
