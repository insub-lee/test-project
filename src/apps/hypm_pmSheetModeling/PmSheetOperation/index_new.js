...
class PmSheetOperationModeling extends PureComponent {
  ...
  render() {
    ...
    const cellClickedReturn = (e) => {
      ...
      if (e.columnGubun === 'MIC_COUNT') { // 점검항목 수
        ...
        (this.state.eqktx === undefined || this.state.eqktx === null || this.state.eqktx === 'null')) {
          ...
          window.open(`/sm/pmSheetModeling/pmSheetMicQualListPopup/${plnal}/${plnnr}/${vornr}/${revision}/-1/-1/${ltxa1}/${ktex1}/${encodeURI(encodeURIComponent(this.state.status))}/${encodeURI(encodeURIComponent(this.state.uid))}/${encodeURI(encodeURIComponent(e.OPGUBUN))}`, '',
          ...
          window.open(`/sm/pmSheetModeling/pmSheetMicQualListPopup/${plnal}/${plnnr}/${vornr}/${revision}/${encodeURI(encodeURIComponent(this.state.eqktx))}/${encodeURI(encodeURIComponent(this.state.beber))}/${ltxa1}/${ktex1}/${encodeURI(encodeURIComponent(this.state.status))}/${encodeURI(encodeURIComponent(this.state.uid))}/${encodeURI(encodeURIComponent(e.OPGUBUN))}`,
          '',
          `width=1200, height=540, top=${screen.availHeight/2-800/2},left=${screen.availWidth/2-705/2},location=no,toolbar=no,menubar=no,scrollbars=false,resizable=yes`); //eslint-disable-line
        }
      } else if (e.columnGubun === 'SP_COUNT') { // 자재 예약 항목
        window.open(`/sm/pmSheetModeling/pmSheetSpListPopup/${plnal}/${plnnr}/${vornr}/${revision}`, '', `width=1400,height=700,top=${screen.availHeight/2-800/2},left=${screen.availWidth/2-705/2},scrollbars=false`); //eslint-disable-line
      }
    };
    ...
    const workTimeClickedReturn = () => {
      ...
      // if(m167_tabDiv == "FAB" && (selected_verwe == 'F1' || selected_verwe == 'F2') ){
      // }else{
      //   alert('전공정 TBM과 CBM만 설정 가능합니다');
      //   return;
      // }
      ...
      window.open(
        `/sm/pmSheetModeling/pmSheetWrokTimePopup/${encodeURI(encodeURIComponent(this.state.plnnr))}/${encodeURI(encodeURIComponent(this.state.plnal))}/${encodeURI(encodeURIComponent(this.state.revision))}`,
        'workTime',
      `top=${screen.availHeight/2-500/2}, left=${screen.availWidth/2-1050/2}, width=560, height=330, location=no, toolbar=no, menubar=no, scrollbars=false, resizable=yes` //eslint-disable-line
      ); //eslint-disable-line
    };
    ...
    const m167OperationFncSave = () => {
      ...
      let saveData = m167OperationGetSaveData(node.data);
      console.log('saveData: ', saveData);
      if (saveData.ROW_TYPE === 'ADD' || saveData.ROW_TYPE === 'EDIT') { // edit일때도 해야함.
        if (saveData.OP_GUBUN === '자사용') {
          saveData.OP_GUBUN = 'A';
        } else if (saveData.OP_GUBUN === '도급사용') {
          saveData.OP_GUBUN = 'B';
        } else if (saveData.OP_GUBUN === '' || saveData.OP_GUBUN === undefined || saveData.OP_GUBUN === null || saveData.OP_GUBUN === 'null') {
          feed.error('용도 구분을 선택해주세요.');
          return;
        } else if (saveData.KTEX1 === '' || saveData.KTEX1 === undefined || saveData.KTEX1 === null || saveData.KTEX1 === 'null') {
          feed.error('PM 주기를 선택해주세요.');
          return;
        } else if (saveData.LTXA1 === '' || saveData.LTXA1 === undefined || saveData.LTXA1 === null || saveData.LTXA1 === 'null') {
          feed.error('Operation Description을 선택해주세요.');
          return;
        }
      }
        if(Number(saveData.VORNR) > 9999){
        ...
        if( saveData.ROW_TYPE !== undefined && saveData.ROW_TYPE === 'ADD' ) {
          this.gridApi.forEachNodeAfterFilter((node, index) => {
            if( node.selected !== true && node.data.VORNR === saveData.VORNR) {
          ...
          addList.push(saveData);
        } else if ( saveData.ROW_TYPE !== undefined && saveData.ROW_TYPE === 'EDIT' ) {
          updateList.push(saveData);
        }
      });
