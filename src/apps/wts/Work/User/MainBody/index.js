...
import WorkerScheduleManageModal from '../../Modals/WorkerScheduleManageModal';
...
class MainBody extends React.Component {
  constructor(props) {
    ...
    this.workerScheduleManageModal = React.createRef();
  ...
  initData() {
    ...
    this.fetchHasInfo(profile.emrno, moment(new Date()).format('YYYYMMDD')).then(payload => {
      this.setState({ hasInfo: payload.hisOXInfo !== null });
    });
    this.setState({ isLoading: true, manHisInfoList: [] }, () => {
      this.fetchWorkList(profile.emrno, moment(new Date()).format('YYYYMM')).then(payload => {
  ...
  handleChangeDate(momentDate) {
    const { profile } = this.props;
    this.setState({ isLoading: true, manHisInfoList: [] }, () => {
      this.fetchWorkList(profile.emrno, momentDate.format('YYYYMM')).then(payload => {
        this.setState({ ...payload, isLoading: false });
      });
    });
  }
  ...
  handleOpenModal(type) {
    switch (type) {
      case 0:
        ...
        break;
      case 1:
        this.workerScheduleManageModal.current.handleOpenModal();
        break;
      case 2:
        ...
        {canUseThisPage ? (
          <>
            ...
            <div className="btn_wrap">
              ...
              <Button type="button" size="small" color="primary" onClick={() => this.handleOpenModal(1)}>
                휴가 등록
              </Button>
            </div>
            ...
            <WorkerScheduleManageModal
              ref={this.workerScheduleManageModal}
              empNo={profile.emrno}
              usrNm={profile.usrnm}
              banjangId={manInfo.banjangId}
              onClose={this.initData}
            />
