...
class ChiefWorkerRecordWriterModal extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      isOpen: false,
    ...
    this.fetchWorkersByArea = this.fetchWorkersByArea.bind(this);
    this.handleChangeWorkJo = this.handleChangeWorkJo.bind(this);
    this.handleChangeUsrNm = this.handleChangeUsrNm.bind(this);
  }

  handleOpenModal(area) {
    this.setState({ isOpen: true, area }, () => {
      this.initData();
    });
  }
  ...
  initData() {
    ...
    this.fetchWorkersByArea(area).then(workers => {
      // Todo - Make WorkJo Tree
      const workJos = [];
      workers.forEach(worker => {
        const workJoIndex = workJos.findIndex(workJo => workJo.key === worker.workjo);
        if (workJoIndex < 0) {
          workJos.push({ key: worker.workjo, label: worker.workjo, children: [{ key: worker.empno, label: worker.usrnm }] });
        } else {
          workJos[workJoIndex].children.push({ key: worker.empno, label: worker.usrnm });
        }
      });
      this.setState(prevState => {
        const { category } = prevState;
        category.workJo = workJos;
        return { category };
      });
    });
  }

  async fetchWorkersByArea(area) {
    const requestQuery = {
      type: 'workersByArea',
      area,
    };
    const { response, error } = await service.manHisChief.get(jsonToQueryString(requestQuery));
    if (response && !error) {
  ...
  handleChangeWorkJo(e) {
    const { value } = e.target;
    this.setState({ selectedWorkJo: value, selectedUsrNm: '' });
  }

  handleChangeUsrNm(e) {
    const { options } = e.target;
    const { selectedIndex } = options;
    this.setState({ selectedUsrNm: selectedIndex > 0 ? options[selectedIndex].text : '' });
  }

  render() {
    const { isOpen, category, diffTime, writeOverWork, wstValue, wetValue, selectedWorkJo, selectedUsrNm } = this.state;
    const { site } = this.props;
    // const { empNo, usrNm, workJo } = this.props;
    const selectedIndex = category.workJo.findIndex(item => item.key === selectedWorkJo);
    return (
      <Modal
        maskClosable={false}
        visible={isOpen}
          ...
          <StyledContent>
            ...
            <div className="pop_con">
              <StyledCommonForm onSubmit={this.submitData} autoComplete="off">
                ...
                <input type="hidden" name="bsign" value="O" />
                ...
                <ul className="sub_form small2 has_margin">
                  ...
                  <li>
                    ...
                    <select name="workJo" id="select-workShift" placeholder="근무조를 선택해주세요." defaultValue="" onChange={this.handleChangeWorkJo}>
                      <option value="">근무조를 선택해주세요.</option>
                      {category.workJo.map(option => (
                  ...
                  <li>
                    ...
                    <input type="hidden" name="usrNm" value={selectedUsrNm} />
                    <select name="empNo" id="select-workShift" defaultValue="" onChange={this.handleChangeUsrNm}>
                      <option value="">근무자를 선택해주세요.</option>
                      {category.workJo[selectedIndex] &&
                        category.workJo[selectedIndex].children.map(option => (
                          <option key={option.key} value={option.key}>
                            {option.label}
                          </option>
                        ))}
                ...
                <div className="sub_form_tit cr">
                  ...
                  <div style={{ float: 'right' }}>
                    <Checkbox className="checkbox" id="use-yn" checked={writeOverWork} onChange={this.handleChangeWriteOverWork} labelText="작성" noPadding />
                  </div>
                  <div className="cr" />
                </div>
                {writeOverWork && (
...
ChiefWorkerRecordWriterModal.propTypes = {
  callbackHandler: PropTypes.func,
  site: PropTypes.string,
};

ChiefWorkerRecordWriterModal.defaultProps = {
  callbackHandler: () => false,
  site: '',
};

export default ChiefWorkerRecordWriterModal;
