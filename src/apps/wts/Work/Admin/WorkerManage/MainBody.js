...
const reorderWorkers = data => {
  const chiefData = data.filter(({ area, bay }) => bay === '반장');
  const prefixData = data.filter(({ area, bay }) => bay !== '반장');
  return chiefData.concat(prefixData);
};
...
class MainBody extends React.Component {
    ...
    this.setState({ isLoading: true, manBanList: [] }, () => {
      this.fetchWorkerList(manInfo.site).then(payload => {
        ...
        data.forEach(row => {
          ...
          if (!manBanArea[area]) manBanArea[area] = [];
          if (!manBanArea[area].includes(row.workjo)) manBanArea[area].push(row.workjo);
        });
        ...
        this.setState({
          ...
          isLoading: false,
          manBanArea,
          currentManBanArea: admin[Number(manInfo.empno)] ? admin[Number(manInfo.empno)].area[0] : '',
        });
      });
    });
  }
  ...
  handleChangeWorkjo(e) {
    ...
    this.setState({ isLoading: true, currentJo: value, manBanList: [] }, () => {
      ...
      const { manInfo } = this.props;
      this.fetchWorkerList(manInfo).then(payload => {
        this.setState({ ...payload, isLoading: false });
      });
    });
  }
  ...
  async fetchWorkerList(site) {
    ...
    if (response && !error) {
      ...
      const nextManBanList = reorderWorkers(
        orderBy(
          manBanList.filter(({ workjo }) => workjo !== '정상조' && workjo !== '통상').map(man => ({ ...man, isChief: man.bay === '반장' ? 1 : 0 })),
          ['area', 'workjo', 'isChief', 'bay'],
          ['asc', 'asc', 'desc', 'asc'],
        ),
      );
      payload.manBanList = nextManBanList;
      const tree = [];
      workBanList
  ...
  openWorkJoSelector(rowData) {
    ...
    this.adminWorkerGroupManageModalRef.current.handleOpenModal(profile.EMP_NO, rowData, tree);
  }

  getFilterData() {
    const { manBanList, filterWord, currentManBanArea, currentManBanJo } = this.state;
    let filterData = manBanList;
    if (currentManBanArea && currentManBanArea !== '') {
      filterData = filterData.filter(row => row.area.includes(currentManBanArea));
    }
    if (currentManBanJo && currentManBanJo !== '') {
      filterData = filterData.filter(row => row.workjo === currentManBanJo);
    }
    if (filterWord.trim().length > 0) {
      filterData = filterData.filter(worker => worker.usrnm.includes(filterWord) || worker.empno.includes(filterWord));
    }
    return filterData;
  }

  render() {
    const { newManList, manJoList, isLoading, checkedList, tree, manBanArea, currentManBanArea, currentManBanJo } = this.state;
    const columns = [
      {
        label: '',
        dataKey: 'usrid',
    ...
    const currentList = this.getFilterData();
    return (
      <Wrapper>
        <StyledStandard>
          <div className="sub_con">
            ...
            <div className="btn_wrap">
              {checkedList.length > 0 && (
                <StyledButton type="button" className="btn-sm btn-gray" onClick={this.openWorkJoSelectorModal}>
                  근무조 지정하기
                </StyledButton>
              )}
            </div>
