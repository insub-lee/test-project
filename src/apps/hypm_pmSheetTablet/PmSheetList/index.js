...
class PmMobile extends React.Component {
  constructor(props) {
    ...
    this.props.handleLoadingSdptList();
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (nextProps.detailData.size === undefined && this.state.sheetNum !== undefined) {
      if (pathArray[1] === 'sm' && history.location.pathname !== '/sm/pmSheetTablet/pmSheet') {
        this.props.history.push({
          pathname: '/sm/pmSheetTablet/pmSheet', state: this.state.sheetNum, props: nextProps.detailData,
        });
  ...
  onAddmonth = () => {
    ...
    const startDate = moment(endDate).add('month', month).format('YYYYMMDD');

    const param = {
      IV_BEBER: '121',
      IV_GSTRP_BEG: startDate,
      IV_GSTRP_END: endDate,
      IV_STATUS: 'T',
      list: {
        IT_STORT: [
          {
  ...
  selectedSheet(value, stat, informId, pmstatus) {
    if (stat !== '송부완료') {
      const param = {
        IV_MODE: 'S',
        IV_INSPLOT: value,
        ...
        CO_STATUS: pmstatus,
        INFORMNOTE_ID: informId +'',
        INSP_LOT_STATUS: stat,
      };

      
      // this.props.handleSavePmStatus(pmstatus, value);
  ...
  render() {
    ...
    return (
      <div>
        ...
        />
        <button onClick={() => this.onAddmonth()}>이전 3개월 더 보기</button>
      </div>
    );
  }
}
...
export function mapDispatchToProps(dispatch) {
  return {
    ...
    handleLoadingSdptList: () => dispatch(actions.getPmSheetSdptList()),
    handleSavePmStatus: (value, num) => dispatch(actions.savePmStatus(value, num)),
  };
}
