...
const selectDraftBase = state => state.get('apps.WorkFlow.WorkFlowBase');
...
const makeApprovalProcessQueId = () =>
  createSelector(
    selectDraftBase,
    state => state.get('approvalProcessQueId'),
  );
...
const makeStepLine = () =>
  createSelector(
    selectDraftBase,
    state => state.get('stepLine').toJS(),
  );

const makeExtraApiData = () =>
  createSelector(
    selectDraftBase,
    state => (state.get('extraApiData') !== undefined ? state.get('extraApiData').toJS() : {}),
  );
