...
const getTreeData = flatData => {
  console.debug('data >> ', flatData);
  console.debug('data2 >> ', getTreeFromFlatData({ flatData, getKey: node => node.NODE_ID, getParentKey: node => node.PARENT_NODE_ID, rootKey: -1 }));
  return getTreeFromFlatData({ flatData, getKey: node => node.NODE_ID, getParentKey: node => node.PARENT_NODE_ID, rootKey: -1 });
};
          ...
          <SortableTree
            ...
            onMoveNode={({ treeData, node, nextParentNode }) => {
              if (node.LVL !== 0) {
                ...
                const { NODE_ID } = node;
                let PARENT_NODE_ID = -1; // 최상위 루트

                if (nextParentNode) {
                  // 부모가 있는 경우 PRNT_ID지정
                  PARENT_NODE_ID = nextParentNode.NODE_ID;
                }
