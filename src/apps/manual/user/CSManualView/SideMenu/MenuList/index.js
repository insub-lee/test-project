...
const setScroll = (compIdx, setSelectedCompIdx, scrollComp, widgetId, bookmarkWidgetData) => {
  ...
  const { appCount, widgetYn } = bookmarkWidgetData;

  if (widgetYn && appCount === 1) {
    scrollComp.scrollTop(scrollTop + topPosition - 181);
  } else if (widgetYn && appCount > 1) {
    scrollComp.scrollTop(scrollTop + topPosition - 181);
  } else {
    scrollComp.scrollTop(scrollTop + topPosition - 141);
  }
