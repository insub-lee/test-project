// get
export const GET_MANUAL_VIEW_SAGA = 'apps/manual/user/ManualView/GET_MANUAL_VIEW_SAGA';

// add

// set
export const SET_MANUAL_VIEW_REDUCR = 'apps/manual/user/ManualView/SET_MANUAL_VIEW_REDUCR';
export const SET_SELECTED_MUAL_IDX_REDUCR = 'apps/manual/user/ManualView/SET_SELECTED_MUAL_IDX_REDUCR';
export const SET_SELECTED_TAB_IDX_REDUCR = 'apps/manual/user/ManualView/SET_SELECTED_TAB_IDX_REDUCR';
export const SET_SELECTED_COMPONENT_IDX_REDUCR = 'apps/manual/user/ManualView/SET_SELECTED_COMPONENT_IDX_REDUCR';
export const SET_SCROLL_COMPONENT_REDUCR = 'apps/manual/user/ManualView/SET_SCROLL_COMPONENT_REDUCR';
export const SET_MANUAL_VIEW_HISTORY_REDUCR = 'apps/manual/user/ManualView/SET_MANUAL_VIEW_HISTORY_REDUCR';

// remove

// etc
export const RESET_MANUAL_VIEW_REDUCR = 'apps/manual/user/ManualView/RESET_MANUAL_VIEW_REDUCR';
