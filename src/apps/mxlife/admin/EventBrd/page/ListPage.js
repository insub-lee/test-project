...
import ParticipantListPage from '../participant';
...
class ListPage extends Component {
  ...
  // Custom Row Render (로우 클릭 관련 수정)
  renderCompRow = (comp, colData, rowData, visible) => {
    if (comp.CONFIG.property.COMP_SRC && comp.CONFIG.property.COMP_SRC.length > 0 && CompInfo[comp.CONFIG.property.COMP_SRC]) {
      if (comp.NAME_KOR === 'CUSTOM_BTN') {
        return (
          <StyledButton className="btn-gray btn-xs" onClick={() => this.customModalHandler('participant', true, rowData)}>
            참여자 정보
          </StyledButton>
        );
      }
      return (
        <div
          tabIndex={0}
          role="button"
          onClick={() => this.customModalHandler('view', true, rowData)}
          onKeyDown={() => this.customModalHandler('view', true, rowData)}
          style={{ width: '100%' }}
        >
          {CompInfo[comp.CONFIG.property.COMP_SRC].renderer({
            ...comp,
            colData,
            rowData,
            ...this.props,
  ...
  renderParticipantModal = () => {
    const { record } = this.state;
    console.debug('이닛1', record);
    return <ParticipantListPage initData={record} />;
  };
  ...
  render = () => {
    ...
    const { isMultiDelete, StyledWrap, isExcelDown, btnTex, fileName, sheetName, columns, fields, pageSize, isPagingData, modalType } = this.state;
        ...
        <StyledWrap className={viewPageData.viewType}>
          <Sketch {...bodyStyle}>
            ...
            <AntdModal
              ...
              width={modalType === 'participant' ? '60%' : '40%'}
              title={this.state.modalTitle}
              onCancel={() => this.customModalHandler('', false)}
            >
              {modalType === 'view' && this.renderViewModal()}
              {modalType === 'participant' && this.renderParticipantModal()}
            </AntdModal>
          </Sketch>
        </StyledWrap>
      );
