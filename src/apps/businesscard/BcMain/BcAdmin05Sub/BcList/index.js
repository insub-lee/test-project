...
class BcinfoList extends React.Component {
  constructor(prop) {
   ...
   // this.handleKeyPress = this.handleKeyPress.bind(this);
    ...
    //this.Cd140_Row = this.Cd140_Row.bind(this);
      this.Cd120_Row = this.Cd120_Row.bind(this);
  }
...
onRowsDeselected = (rows) => {
 ...
 //---------------------------------------------------------------------------------------------------------------
  ...
  dtlLink = (data) => {
    this.props.history.push({ pathname: `/${basicPath.APPS}/businesscard/BcMain/BcAdmin05Sub/BcDetail`, state: data });
  }
 //-------------------------------------------REQST_KOR_NM--------------------------------------------------------------------
...
//-----발급준비(발급대상)-------120------------------------------------------------------------
Cd120_Confirm = () => {
  feed.showConfirm(`발급준비(대상) 처리 하시겠습니까?`, '', this.Cd120_Row);
}
Cd120_Row = () => {
  this.props.delRow120(this.state.CardStdCd_Id);
  this.initState();
  this.pageInitState();
};
...
//-----발급대rl(발급완료) 130-------------------------------------------------------------
Cd130_Confirm = () => {
  feed.showConfirm(`발급완료 하시겠습니까?`, '', this.Cd130_Row);
}
Cd130_Row = () => {
...
//-----발급완료(수령완료) 130-------------------------------------------------------------
Cd140_Confirm = () => {
  feed.showConfirm(`수령완료 하시겠습니까?`, '', this.Cd140_Row);
}
Cd140_Row = () => {
  this.props.delRow140(this.state.CardStdCd_Id);
  this.initState();
  this.pageInitState();
};
...
//-----메일발송  -------------------------------------------------------------
Mail_Confirm = () => {
  feed.showConfirm(`메일발송 하시겠습니까?`, '', this.Cd700_Row);
}
Cd700_Row = () => {
  this.initState();
  this.pageInitState();
...
//------------------------------------------------------------------------
  ...
  render() {
   ...
   <div className="buttonWrapper" align='center'>
            <React.Fragment>
                <LinkBtnLgtGray onClick={this.Cd120_Confirm}>{' 발급재개(발급대기) '}</LinkBtnLgtGray> &nbsp;&nbsp;&nbsp;&nbsp;
                <LinkBtnLgtGray onClick={this.Cd126_Confirm}>{' 발급취소처리 '}</LinkBtnLgtGray>
            </React.Fragment>
        </div>
...
const mapDispatchToProps = dispatch => (
  {
    ...
    //delRow140: CardStdCd_Id => dispatch(actions.delRow140(CardStdCd_Id)),  //수령완료
      delRow120: CardStdCd_Id => dispatch(actions.delRow120(CardStdCd_Id)),  //발급대상
...
const withReducer = injectReducer({ key: 'SiteList05', reducer });
const withSaga = injectSaga({ key: 'SiteList05', saga });
