...
onRowsDeselected = (rows) => {
 ...
 //---------------------------------------------------------------------------------------------------------------
  ...
  dtlLink = (data) => {
    this.props.history.push({ pathname: `/${basicPath.APPS}/businesscard/BcMain/BcAdmin04Sub/BcDetail`, state: data });
  }
 //-------------------------------------------REQST_KOR_NM--------------------------------------------------------------------
...
//------------------------------------------------------------------------
  ...
  render() {
   ...
   <div className="buttonWrapper" align='center'>
            <React.Fragment>
                <LinkBtnLgtGray onClick={this.Cd120_Confirm}>{' 발급준비(발급대기)) '}</LinkBtnLgtGray>
            </React.Fragment>
        </div>
...
const withReducer = injectReducer({ key: 'SiteList04', reducer });
const withSaga = injectSaga({ key: 'SiteList04', saga });
