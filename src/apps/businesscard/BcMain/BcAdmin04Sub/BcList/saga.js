...
export function* getList(payload) {
  const response = yield call(Axios.post, '/apps/api/v1/bc/BcAdminList04', payload);
  if (response.siteList04 !== null) {
    yield put({
      type: constants.SET_INFO_LIST,
      payload: fromJS(response.siteList04),
    });
  } else {
    yield put({
      type: constants.SET_INFO_LIST,
...
export default function* orgSage() {
 ...
 // yield takeLatest(constants.GET_STD_ROW140, delRow140);  
  yield takeLatest(constants.GET_STD_ROW120, delRow120);  
