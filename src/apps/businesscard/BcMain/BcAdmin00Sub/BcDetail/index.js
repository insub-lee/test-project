...
//  class BcinfoList extends PureComponent {
  ...
  onClickToList = () => {
    ...
    this.props.history.push({
      pathname: `/${basicPath.APPS}/businesscard/BcMain/BcAdmin00Sub`, state: data,
    });
  }
  ...
  //----승인처리-------------------------------------------------------------------
  Cd100_Confirm = () => {
    feed.showConfirm(`승인 하시겠습니까 ?`, '', this.CardStdCd_100);
  }
  CardStdCd_100 = () => {
    const CardStdCd_Id = [this.state.BC_ID];
    this.props.CardStdCd_100(CardStdCd_Id, this.props.history);
  }
  //------------------------------------------------------------------------------
  //CM_CODE   126 CT1600004  발급취소     
  //----발급취소-------------------------------------------------------------------
  Cd126_Confirm = () => {
    feed.showConfirm(`발급취소 하시겠습니까 ?`, '', this.CardStdCd_126);
  }
  CardStdCd_126 = () => {
    const CardStdCd_Id = [this.state.BC_ID];
    this.props.CardStdCd_126(CardStdCd_Id, this.props.history);
  }
  //------------------------------------------------------------------------------
  // 125   CT1600004  발급보류  
  Cd125_Confirm = () => {
    feed.showConfirm(`발급보류 하시겠습니까 ?`, '', this.CardStdCd_125);
  }
  CardStdCd_125 = () => {
    const CardStdCd_Id = [this.state.BC_ID];
    this.props.CardStdCd_125(CardStdCd_Id, this.props.history);
  }
...
//================================================================================================
  render() {
    ...
    return (
      <div> <p>&nbsp;</p><p>&nbsp;</p><p>&nbsp;</p>
       <StyleVGroup>
       <h3 className="pageTitle">{title}  ( {this.props.siteInfoRow.CARD_TYPE_NM} ) {this.props.siteInfoRow.CARD_TYPE_CD} </h3>
       <div className="buttonWrapper">
            <React.Fragment>
              <div style={{ float: 'right' }}><LinkBtnLgtGray onClick={this.onClickToList}>{' 돌아가기 '}</LinkBtnLgtGray></div>
              <div style={{ float: 'right' }}><LinkBtnLgtGray onClick={this.CardStdCd_125}>{' 보류처리 '}</LinkBtnLgtGray></div>
              <div style={{ float: 'right' }}><LinkBtnLgtGray 
                            onClick={() =>
                                          this.props.history.push({ pathname: `/${basicPath.APPS}/businesscard/BcMain/BcAdmin00Sub/BcinfoEdt`, search: 'U', state: bInfo })}>
                                                                                        {' 수 정 '}</LinkBtnLgtGray></div>
              <div style={{ float: 'right' }}><LinkBtnLgtGray onClick={this.Cd126_Confirm}>{' 발급취소 '}</LinkBtnLgtGray></div>
              <div style={{ float: 'right' }}><LinkBtnLgtGray onClick={this.Cd100_Confirm}>{' 승인(접수) '}</LinkBtnLgtGray></div>
...
const mapDispatchToProps = dispatch => (
  {
    ...
    //승인(접수)
    CardStdCd_100: (CardStdCd_Id, history) =>
      dispatch(actions.CardStdCd_100(CardStdCd_Id, history)),

    //발급취소
    CardStdCd_126: (CardStdCd_Id, history) =>
      dispatch(actions.CardStdCd_126(CardStdCd_Id, history)),
    
    //발급보류
    CardStdCd_125: (CardStdCd_Id, history) =>
      dispatch(actions.CardStdCd_125(CardStdCd_Id, history)),
