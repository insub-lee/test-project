...
onRowsDeselected = (rows) => {
 ...
 //---------------------------------------------------------------------------------------------------------------
  ...
  dtlLink = (data) => {
    this.props.history.push({ pathname: `/${basicPath.APPS}/businesscard/BcMain/BcAdmin03Sub/BcDetail`, state: data });
  }
 //-------------------------------------------REQST_KOR_NM--------------------------------------------------------------------
...
//------------------------------------------------------------------------
  ...
  render() {
   ...
   <div className="buttonWrapper" align='center'>
            <React.Fragment>
                <LinkBtnLgtGray onClick={this.Mail_Confirm}> {' 메일발송 '}</LinkBtnLgtGray>
                <LinkBtnLgtGray onClick={this.Cd140_Confirm}>{' 완 료 '}</LinkBtnLgtGray> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
              {/*
...
const withReducer = injectReducer({ key: 'SiteList03', reducer });
const withSaga = injectSaga({ key: 'SiteList03', saga });
