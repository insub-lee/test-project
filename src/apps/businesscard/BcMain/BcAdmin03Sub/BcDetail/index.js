...
//  class BcinfoList extends PureComponent {
  ...
  // 140   CT1600004  수령완료  
  Cd140_Confirm = () => {
     feed.showConfirm(`수령확인 하시겠습니까 ?`, '', this.CardStdCd_140);
  }
  CardStdCd_140 = () => {
     const CardStdCd_Id = [this.state.BC_ID];
     this.props.CardStdCd_140(CardStdCd_Id, this.props.history);
  }
       ...
       <div className="buttonWrapper">
            <React.Fragment>
              ...
              <div style={{ float: 'right' }}><LinkBtnLgtGray 
                            onClick={() =>
                                          this.props.history.push({ pathname: `/${basicPath.APPS}/businesscard/BcMain/BcAdmin03Sub/BcinfoEdt`, search: 'U', state: bInfo })}>
                                                                                        {' 수 정 '}</LinkBtnLgtGray></div>
              <div style={{ float: 'right' }}><LinkBtnLgtGray onClick={this.Cd140_Confirm}>{' 수령확인 '}</LinkBtnLgtGray></div>
...
const mapDispatchToProps = dispatch => (
  {
    ...
    getBcInfo: BC_ID =>      dispatch(actions.getBcInfo(BC_ID)),

    delGlobalMsg: (delKeys, history) =>dispatch(actions.delGlobalMsg(delKeys, history)),
    //승인(접수)
    CardStdCd_100: (CardStdCd_Id, history) =>dispatch(actions.CardStdCd_100(CardStdCd_Id, history)),
    //발급취소
    CardStdCd_126: (CardStdCd_Id, history) =>dispatch(actions.CardStdCd_126(CardStdCd_Id, history)),
    //발급보류
    CardStdCd_125: (CardStdCd_Id, history) =>dispatch(actions.CardStdCd_125(CardStdCd_Id, history)),
    //수령완료
    CardStdCd_140: (CardStdCd_Id, history) =>dispatch(actions.CardStdCd_140(CardStdCd_Id, history)),
  }
);
