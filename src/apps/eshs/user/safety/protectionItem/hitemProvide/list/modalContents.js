...
class ModalContents extends React.Component {
  ...
  setReqCd = () => {
    ...
    changeFormData(sagaKey, 'REQ_CD', (result.lastReqCd && result.lastReqCd.list && result.lastReqCd.list.REQ_CD) || '');
  };
  ...
  saveAfterFunc = () => {
    ...
    const { sagaKey, getDataSource, handleModalClose, submitHandlerBySaga, result } = this.props;
    const REQ_CD = (result.lastReqCd && result.lastReqCd.list && result.lastReqCd.list.REQ_CD) || '';
    ...
    submitHandlerBySaga(sagaKey, 'POST', '/api/eshs/v1/common/protection-req-detail', { PARAM: { list: param } }, submitCallbackFunc);
  };
