...
class CustomModifyPage extends Component {
  constructor(props) {
    super(props);
    this.state = {
      uploadFileList: [],
    ...
    if (viewLayer.length === 1 && viewLayer[0].CONFIG && viewLayer[0].CONFIG.length > 0 && isJSON(viewLayer[0].CONFIG)) {
      ...
      return (
        <StyledViewDesigner>
          <Sketch {...bodyStyle}>
            <SearchBar
              modifySaveTask={customCallBackFunc => this.saveTask(id, reloadId || id, customCallBackFunc)}
              sagaKey={id}
...
export default CustomModifyPage;
