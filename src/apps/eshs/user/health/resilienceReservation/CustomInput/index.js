...
class Comp extends Component {
  ...
  componentDidMount = () => {
    ...
    const apiAry = [
      ...
      },
      {
        key: 'getTimetable',
        type: 'GET',
        url: `/api/eshs/v1/common/getreserveresiliencetimetable?date=${APP_DT}`,
      },
      {
        key: 'noShowUpdate',
        type: 'POST',
      ...
      },
      {
        key: 'weekCount',
        type: 'GET',
        url: `/api/eshs/v1/common/getresiliencereservation?DATE=${APP_DT}&USER_ID=${userId}`,
      },
    ];
    ...
    return this.setState(
      prevState => ({
        formData: {
          ...
          SITE: userInfo.BAREA_CD === '청주' ? 'CP' : 'GP',
        },
        timeTable,
        weekCount,
      }),
  ...
  handleDelete = timeZone => {
    ...
    submitHandlerBySaga(id, 'DELETE', '/api/eshs/v1/common/getreserveresiliencetimetable', { PARAM: { appDt: APP_DT, timeZone } }, (afterId, res) => {
  ...
  handleConfirm = (timeZone, isChk) => {
    ...
    submitHandlerBySaga(id, 'POST', '/api/eshs/v1/common/getreserveresiliencetimetable', { PARAM: { appDt: APP_DT, timeZone, isChk } }, (afterId, res) => {
      ...
      children: [
        {
          ...
          render: (text, record, index) => {
            ...
            const fIdx = timeTable && timeTable.findIndex(user => user.time_zone === text);

            if (fIdx <= -1)
              return (
                <Checkbox
                  onChange={e => this.handleOnCheck(e.target.checked, index, record)}
                  checked={text === timeZone}
                  disabled={moment() > moment(record.text.substring(0, 5), 'HH:mm') && moment().format('YYYY-MM-DD') >= moment(APP_DT).format('YYYY-MM-DD')}
                />
              );
