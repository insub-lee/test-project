...
class StressView extends Component {
  ...
  checkOpenServeyCallback = (id, response) => {
    ...
    const surveydata = info ? this.setSurveyData(info) : {};
    this.setState({
      pollList: list || [],
  ...
  setSurveyResult = (key, data) => (
    <>
      <td>{data && data.length > 0 ? (data.find(item => item.TYPE === '1') && data.find(item => item.TYPE === '1').COUNT) || 0 : 0} 명</td>
      <td>{data && data.length > 0 ? (data.find(item => item.TYPE === '2') && data.find(item => item.TYPE === '2').COUNT) || 0 : 0} 명</td>
      <td>{data && data.length > 0 ? (data.find(item => item.TYPE === '3') && data.find(item => item.TYPE === '3').COUNT) || 0 : 0} 명</td>
      <td>{data && data.length > 0 ? (data.find(item => item.TYPE === '4') && data.find(item => item.TYPE === '4').COUNT) || 0 : 0} 명</td>
    </>
  );

  render() {
    const { isload, formData, pollList, selectedPoll } = this.state;
    if (!isload) return '';
    return (
      <Styled>
