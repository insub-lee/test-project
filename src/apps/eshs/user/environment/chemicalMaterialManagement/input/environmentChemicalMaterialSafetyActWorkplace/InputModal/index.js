...
class InputModal extends React.Component {
  ...
  handleSearchChange = (e, type, isSub) => {
    ...
    if (type.toUpperCase() === 'SELECT' && !isSub) {
      this.setState(
        {
          CATEGORY_ID: e,
          SUB_CATEGORY_ID: '',
  ...
  getSearchList = () => {
    const { CATEGORY_ID, SUB_CATEGORY_ID, KEYWORD } = this.state;
    const { sagaKey: id, getCallDataHandler, apiUrl } = this.props;
    const apiArr = [
      {
        key: 'materialList',
        type: 'GET',
        url: `${apiUrl}?CATEGORY_ID=${CATEGORY_ID}&SUB_CATEGORY_ID=${SUB_CATEGORY_ID}&KEYWORD=${KEYWORD}`,
      },
    ];
  ...
  render() {
    ...
    const { KEYWORD, CATEGORY_ID, SUB_CATEGORY_ID } = this.state;
