...
import sampleExcelFile from './sampleExcel/수계(양식).xlsx';
...
class ExcelParser extends Component {
  ...
  excelDataSet = data => {
    ...
    data.forEach((row, index) => {
      if (index > 3 && row[0] !== '') {
        const rowData = {
          EI_SYSTEM: `${row[0]}`, // 시스템 M
          INCOM_TON: `${row[1]}`, // 발생량
          INCOM_POLLUTION: `${row[2]}`, // 유입 오염물질
          INCOM_SPEC: `${row[3]}`, // SPEC
          INCOM_AV: `${row[4]}`, // 발생농도 AV.
          INCOM_HUNTING: `${row[5]}`, // 발생농도 HUNTING
          DISCHARGE: `${row[6]}`, // 배출구
          DISCHARGE_TON: `${row[7]}`, // 배출량
          DISCHARGE_POLLUTION: `${row[8]}`, // 배출 오염물질
          DISCHARGE_WARNING: `${row[9]}`, // WARNING
          DISCHARGE_SHOTDOWN: `${row[10]}`, // SHOTDOWN
          DISCHARGE_AV: `${row[11]}`, // 배출농도 AV.
          DISCHARGE_HUNTING: `${row[12]}`, // 배출농도 HUNTING
          DISCHARGE_TOTAL: `${row[13]}`, // 배출농도 HUNTING
        };
        result.push(rowData);
      }
    });
