...
class SendMailListComp extends Component {
  constructor(props) {
    super(props);
    this.state = {
      searchParam: {
        REQ_DT: toDay,
        GET_EMP_NO: this.props.profile.EMP_NO || undefined,
      },
      modalObj: {
        title: '',
        visible: false,
    ...
    const apiAry = [
      {
        key: 'list',
        type: 'POST',
        url: '/api/eshs/v1/common/eshsSafetyImproveSendMailList',
        ...
        <StyledContentsWrapper>
          <StyledCustomSearchWrapper className="search-wrapper-inline">
            <div className="search-input-area">
              <span className="text-label">발송일자</span>
              <AntdDatePicker
                className="ant-picker-sm mr5 ml5"
                style={{ width: '150px' }}
                defaultValue={moment(toDay)}
                allowClear={false}
                onChange={(date, strDate) => this.changeSearchParam('REQ_DT', strDate)}
              />
              {isAdmin && (
                <AntdSearchInput
                  style={{ width: '150px' }}
                  className="input-search-sm mr5"
                  value={searchParam.GET_EMP_NO || undefined}
                  placeholder="요청자 사번"
                  onSearch={() =>
                    this.changeModalObj('사원 검색', true, [
                      <UserSearchModal
                        ...
                        visible
                        onClickRow={data => {
                          this.changeSearchParam('GET_EMP_NO', data.EMP_NO);
                          this.changeModalObj();
                        }}
                      />,
                    ])
                  }
                  onChange={e => this.changeSearchParam('GET_EMP_NO', e.target.value)}
                />
              )}
            </div>
...
export default SendMailListComp;
