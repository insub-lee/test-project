...
class Hazard extends Component {
  ...
  handleCustomAction = (id, formData, changeFormData, afterCallBack, type, validationCkNeed) => {
    ...
    if(endYn === 'Y'){
      message.info(<MessageContent>완료가 된 위험요인 List-up 입니다. 해당항목은 수정 및 저장이 불가능합니다.</MessageContent>);
      return false;
    }
  ...
  modifyCustomButtons = ({ ...modifyProps }) => {
    ...
    return (
      <>
        <StyledButton className="btn-primary btn-first btn-xs" onClick={() => this.handleCustomAction( id , formData , changeFormData , () => onChangeSave('M'), '','N')}>
          저장
        </StyledButton>
        <Popconfirm
          title="정말로 삭제하시겠습니까?"
        ...
        <StyledButton className="btn-primary btn-xs" onClick={() => this.handleCustomAction(id, formData, changeFormData, () => onChangeSave('M'), 'Y','Y')}>
          완료
        </StyledButton>
      </>
    );
