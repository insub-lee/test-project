...
class List extends Component {
  constructor(props) {
    super(props);
    this.state = {
      rangeData: [moment(moment().subtract(1, 'years')).format('YYYY-MM-DD'), moment().format('YYYY-MM-DD')],
    };
  }
  ...
  initData = () => {
    ...
    const selectData = (selectList && selectList.categoryMapList.slice(1)) || [];
  ...
  search = () => {
    ...
    param += empNo ? `&&REG_EMPNO=${empNo}` : '';
    param += empNm ? `&&REG_EMPNM=${empNm}` : '';
    ...
    return (
      <StyledContentsWrapper>
        <StyledCustomSearchWrapper>
          <div className="search-input-area mb10">
            ...
            <AntdRangePicker
              ...
              onChange={(date, dateStrings) => this.setState({ rangeData: dateStrings })}
            />
          </div>
          <div className="search-input-area mb10">
            ...
            <AntdInput
              ...
              placeholder="전체"
              style={{ width: 200 }}
              onChange={e => this.setState({ deptNm: e.target.value })}
            />
            <span className="text-label">작성자 사번</span>
            <AntdInput className="ant-input-sm" allowClear placeholder="전체" style={{ width: 200 }} onChange={e => this.setState({ empNo: e.target.value })} />
            <span className="text-label">작성자 성명</span>
            <AntdInput className="ant-input-sm" allowClear placeholder="전체" style={{ width: 200 }} onChange={e => this.setState({ empNm: e.target.value })} />
          </div>
        ...
        <AntdModal width={1000} visible={this.state.isModal} title="위험성 평가" onCancel={this.onModalChange} destroyOnClose footer={null}>
          {this.state.isModal && <DanestAdmin improveDanger={{ REG_NO: recordByState.REG_NO, REG_DTTM: recordByState.REG_DATE, IMPROVE: true }} />}
        </AntdModal>
      </StyledContentsWrapper>
    );
  }
