...
import { address } from 'apps/eshs/admin/safety/InspectionTarget/FireErt/internal_constants';
...
export default function RegisterInspection({
  ...
  const shoudFireAPI = () => {
    ...
    const { EXTERIOR_YN, SIGN_ATTACH_YN, OPTIMUM_LEVEL_YN, CLEAN_YN, EQUIP_YN } = formData;
    handleModalLoading(true);
    if (EXTERIOR_YN && SIGN_ATTACH_YN && OPTIMUM_LEVEL_YN && CLEAN_YN && EQUIP_YN) {
      const { POSITION_NO, CHIP_NO, REG_USER_ID } = formData;
      request({
        method: 'POST',
        url: `${address.registerInspectionResult}`,
        data: { EXTERIOR_YN, SIGN_ATTACH_YN, OPTIMUM_LEVEL_YN, CLEAN_YN, EQUIP_YN, POSITION_NO, CHIP_NO, REG_USER_ID },
      }).then(({ response }) => {
