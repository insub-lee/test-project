...
import { address, VIEW_TYPE, META_SEQ } from 'apps/eshs/admin/safety/InspectionTarget/FireWhydrant/internal_constants';
...
const ListPage = props => {
  ...
  const modalTitle = () => {
    if (activateRegModal) return '미분무 소화전 신규등록';
    if (activateDetailModal) return '미분무 소화전 점검';
    if (activateUsageModal) return '미분무 소화전 사용/미사용 등록';
    return '';
  };
