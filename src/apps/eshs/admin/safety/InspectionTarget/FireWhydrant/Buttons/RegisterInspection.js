...
import { address } from 'apps/eshs/admin/safety/InspectionTarget/FireWhydrant/internal_constants';
...
export default function RegisterInspection({
  ...
  const shoudFireAPI = () => {
    ...
    const { POWER_YN, EXTERIOR_YN, HOSE_GUN_YN } = formData;
    if (POWER_YN && EXTERIOR_YN && HOSE_GUN_YN) {
      const { POSITION_NO, CHIP_NO, REG_USER_ID } = formData;
      request({
        method: 'POST',
        url: `${address.registerInspectionResult}`,
        data: { POWER_YN, EXTERIOR_YN, HOSE_GUN_YN, POSITION_NO, CHIP_NO, REG_USER_ID },
      }).then(({ response }) => {
