...
import { address } from 'apps/eshs/admin/safety/InspectionTarget/FirePkg/internal_constants';
...
export default function RegisterInspection({
  ...
  const shoudFireAPI = () => {
    ...
    const { DEFENSE_YN, SOL_YN, CONTROL_YN } = formData;
    handleModalLoading(true);
    if (DEFENSE_YN && SOL_YN && CONTROL_YN) {
      const { POSITION_NO, CHIP_NO, REG_USER_ID } = formData;
      request({
        method: 'POST',
        url: `${address.registerInspectionResult}`,
        data: { DEFENSE_YN, SOL_YN, CONTROL_YN, POSITION_NO, CHIP_NO, REG_USER_ID },
      }).then(({ response }) => {
