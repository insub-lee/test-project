...
import { address } from 'apps/eshs/admin/safety/InspectionTarget/FireWpump/internal_constants';
...
export default function RegisterInspection({
  ...
  const shoudFireAPI = () => {
    ...
    const { SLAKING_WATER_YN, WATER_LEAK_NOISE_YN, CONTROL_VALVE_YN, RATED_FLUX_YN } = formData;
    handleModalLoading(true);
    if (SLAKING_WATER_YN && WATER_LEAK_NOISE_YN && CONTROL_VALVE_YN && RATED_FLUX_YN) {
      const { POSITION_NO, CHIP_NO, REG_USER_ID } = formData;
      request({
        method: 'POST',
        url: `${address.registerInspectionResult}`,
        data: { SLAKING_WATER_YN, WATER_LEAK_NOISE_YN, CONTROL_VALVE_YN, RATED_FLUX_YN, POSITION_NO, CHIP_NO, REG_USER_ID },
      }).then(({ response }) => {
