...
import { address } from 'apps/eshs/admin/safety/InspectionTarget/FireEbShower/internal_constants';
...
export default function RegisterInspection({
  ...
  const shoudFireAPI = () => {
    ...
    const { LAVATION_YN, WATER_PRESSURE_YN, DRANIN_YN, EXTERIOR_YN, INDICATO_BOARD_YN } = formData;
    if (LAVATION_YN && WATER_PRESSURE_YN && DRANIN_YN && EXTERIOR_YN && INDICATO_BOARD_YN) {
      const { POSITION_NO, CHIP_NO, REG_USER_ID } = formData;
      request({
        method: 'POST',
        url: `${address.registerInspectionResult}`,
        data: { LAVATION_YN, WATER_PRESSURE_YN, DRANIN_YN, EXTERIOR_YN, INDICATO_BOARD_YN, POSITION_NO, CHIP_NO, REG_USER_ID },
      }).then(({ response }) => {
