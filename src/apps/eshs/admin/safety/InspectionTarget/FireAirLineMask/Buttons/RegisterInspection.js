...
import { address } from 'apps/eshs/admin/safety/InspectionTarget/FireAirLineMask/internal_constants';
...
export default function RegisterInspection({
  ...
  const shoudFireAPI = () => {
    ...
    const { EXTERIOR_YN, CLEANLINESS_YN, OPTIMUM_LEVEL_YN, CONECTO_YN } = formData;
    handleModalLoading(true);
    if (EXTERIOR_YN && CLEANLINESS_YN && OPTIMUM_LEVEL_YN && CONECTO_YN) {
      const { POSITION_NO, CHIP_NO, REG_USER_ID } = formData;
      request({
        method: 'POST',
        url: `${address.registerInspectionResult}`,
        data: { EXTERIOR_YN, CLEANLINESS_YN, OPTIMUM_LEVEL_YN, CONECTO_YN, POSITION_NO, CHIP_NO, REG_USER_ID },
      }).then(({ response }) => {
