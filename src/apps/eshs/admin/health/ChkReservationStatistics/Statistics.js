...
class Statistics extends Component {
  ...
  getStatistics = () => {
    ...
    const apiAry = [
      {
        key: 'statistics',
        url: '/api/eshs/v1/common/health/healthChkReservationStatistics',
        type: 'POST',
        params: {
  ...
  initStatistics = () => {
    ...
    if (result && result.statistics && result.statistics.list) {
      ...
      let hosCurrChkSeq = '';
      this.setState({
        list: result.statistics.list.map((item, idx) => {
          if (currWork !== item.WORK_AREA_CD_NODE_ID) {
            groupCnt = result.statistics.list.filter(l => l.WORK_AREA_CD_NODE_ID === item.WORK_AREA_CD_NODE_ID).length;
          } else {
            groupCnt = 0;
          }
          currWork = item.WORK_AREA_CD_NODE_ID;
          return {
            ...item,
            rowSpan: groupCnt,
          }
        }),
        hosList: result.statistics.hosList.map((item, idx) => {
          // 지역 rowSpan 구하기
          if (hosCurrWork !== item.WORK_AREA_CD_NODE_ID) {
            hosWorkCnt = result.statistics.hosList.filter(l => l.WORK_AREA_CD_NODE_ID === item.WORK_AREA_CD_NODE_ID).length;
          } else {
            hosWorkCnt = 0;
          }

          // 검진종류 rowSpan 구하기
          if (hosCurrWork !== item.WORK_AREA_CD_NODE_ID || (hosCurrWork === item.WORK_AREA_CD_NODE_ID && hosCurrChkType !== item.CHK_TYPE_CD_NODE_ID)) {
            hosChkTypeCnt = result.statistics.hosList.filter(
              l => l.WORK_AREA_CD_NODE_ID === item.WORK_AREA_CD_NODE_ID && l.CHK_TYPE_CD_NODE_ID === item.CHK_TYPE_CD_NODE_ID).length;
          } else {
            hosChkTypeCnt = 0;
          }

          // 검진차수 rowSpan 구하기
          if (hosCurrWork !== item.WORK_AREA_CD_NODE_ID || hosCurrChkType !== item.CHK_TYPE_CD_NODE_ID || 
              (hosCurrWork === item.WORK_AREA_CD_NODE_ID && hosCurrChkType === item.CHK_TYPE_CD_NODE_ID && hosCurrChkSeq !== item.CHK_SEQ)) {
            hosChkSeqCnt = result.statistics.hosList.filter(
              l => l.WORK_AREA_CD_NODE_ID === item.WORK_AREA_CD_NODE_ID 
              && l.CHK_TYPE_CD_NODE_ID === item.CHK_TYPE_CD_NODE_ID
              && l.CHK_SEQ === item.CHK_SEQ).length;
          } else {
            hosChkSeqCnt = 0;
          }

          hosCurrWork = item.WORK_AREA_CD_NODE_ID;
          hosCurrChkType = item.CHK_TYPE_CD_NODE_ID;
          hosCurrChkSeq = item.CHK_SEQ;
          return {
            ...item,
            rowSpan1: hosWorkCnt,
            rowSpan2: hosChkTypeCnt,
            rowSpan3: hosChkSeqCnt,
          }
        })
      });
    }
  ...
  render() {
    ...
    return (
      <StyledContentsWrapper>
        ...
        <Typography.Text>1. 검종별 인원 및 예약/검진률</Typography.Text>
        <StyledHtmlTable style={{ marginBottom: 20 }}>
          <table>
            ...
            {this.state.list && this.state.list.map(item => (
              ...
              {!item.CHK_TYPE_CD_NODE_ID && !item.CHK_SEQ ? (
                <td colSpan={!item.WORK_AREA_CD_NODE_ID ? 3 : 2}>{!item.WORK_AREA_CD_NODE_ID ? '합계' : '소계'}</td>
              ) : (
                item.rowSpan !== 0 && <td rowSpan={item.rowSpan}>{item.WORK_AREA_NAME}</td>
              )}
                {item.CHK_TYPE_CD_NODE_ID && <td>{item.CHK_TYPE_NAME}</td>}
                {item.CHK_SEQ && <td>{item.CHK_SEQ === '1' ? '1차' : '재검'}</td>}
            ...
            {this.state.hosList && this.state.hosList.map(item => (
              ...
              {!item.CHK_TYPE_CD_NODE_ID && !item.CHK_SEQ && !item.HOSPITAL_CODE ? (
                ...
                <>
                  ...
                  {item.rowSpan2 !== 0 && <td rowSpan={item.rowSpan2}>{item.CHK_TYPE_NAME}</td>}
                  {item.rowSpan3 !== 0 && <td rowSpan={item.rowSpan3}>{item.CHK_SEQ === '1' ? '1차' : '재검'}</td>}
                </>
              )}
