...
import StyledTag from 'commonStyled/Tag/StyledTag';
import StyledTagWrapper from 'commonStyled/Tag/StyledTagWrapper';
...
class CustomerEmailSend extends Component {
  ...
  render() {
    ...
    let list = result && result.distDeptList && result.distDeptList.list ? result.distDeptList.list.filter(item => item.DEPT_ID === 1461 || item.PRNT_ID === 1461) : [];
    ...
    return (
      <>
                    ...
                    <StyledTagWrapper>
                      <div className="ant-tag-inner">
                        {bccList && bccList.length > 0 && (
                          bccList.map(user => (
                            <AntdTag closable onClose={e => this.onCloseTag(e, user)}>{user.EMAIL}</AntdTag>
                          ))
                        )}
                      </div>
                      <StyledButton className="btn-light btn-xs" onClick={this.onUserSelectModal}>선택</StyledButton>
                    </StyledTagWrapper>
                  </td>
                  <td>
                  </td>
        ...
        <StyledButtonWrapper className="btn-wrap-center">
          <StyledButton className="btn-primary" onClick={e => this.onClickSendMail(e)}>전송</StyledButton>
        </StyledButtonWrapper>
