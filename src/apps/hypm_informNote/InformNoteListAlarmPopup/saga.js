...
export function* alarmSearch(payload) {
  const response1 = yield call(Axios.post, '/api/gipms/v1/informNote/fabInformNoteListAlarmPopupSearch', payload.param);

  yield put({
    type: constants.LOADING_ALARM_SEARCH,
    alarmData: response1.list.alarmData,
  });
}

export function* alarmSave(payload) {
  const response1 = yield call(Axios.post, '/api/gipms/v1/informNote/fabInformNoteListAlarmPopupSave', payload.param);
...
export default function* alarmSaga() {
  ...
  yield takeLatest(constants.LOADING_ALARM_SEARCH_SAGA, alarmSearch);
  yield takeLatest(constants.LOADING_ALARM_SAVE_SAGA, alarmSave);
}
