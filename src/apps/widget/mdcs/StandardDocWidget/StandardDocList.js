...
class StandardDocList extends Component {
  ...
  componentDidMount() {
    ...
    submitHandlerBySaga(sagaKey, 'POST', url, { PARAM: { PAGE: 1, PAGE_CNT: 5 } }, this.initDataBind);
  }

  initDataBind = (sagaKey, response) => {
    if (response && response.list && response.list.length > 0) {
      const standardDocList = response.list;
      this.setState({ standardDocList });
    }
  };
  ...
  render() {
    const { visible, viewType, workSeq, taskSeq, standardDocList, coverView, modalList, listVisible } = this.state;
    const { widgetTitle, widgetClassName } = this.props;
    return (
