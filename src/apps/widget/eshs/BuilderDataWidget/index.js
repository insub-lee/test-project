...
class BuilderDataList extends Component {
  ...
  render() {
    ...
    return (
      <>
        <AntdModal width={700} visible={this.state.isShow} title={(item && item.NAME_KOR) || ''} onCancel={this.onCancelPopup} destroyOnClose footer={null}>
          <BizBuilderBase
            ...
            ViewCustomButtons={() => (
              <StyledButtonWrapper className="btn-wrap-center btn-wrap-mt-20">
                <StyledButton className="btn-light btn-sm" onClick={this.onCancelPopup}>
                  닫기
                </StyledButton>
              </StyledButtonWrapper>
            )}
        ...
        <StyledBuilderDataWidget>
          ...
          <StyledButtonWrapper className="btn-wrap-center">
            <StyledButton className="btn-link" onClick={() => history.push(`${item.data.MORE_URL}`)}>
              <Icon type="plus" /> 더보기
            </StyledButton>
          </StyledButtonWrapper>
        </StyledBuilderDataWidget>
...
class BuilderDataWidget extends Component {
  render() {
    ...
    return (
      <BizBuilderBase
        sagaKey={`BuilderBasicList_${item.WIDGET_ID}`}
        viewType="LIST"
        CustomListPage={BuilderDataList}
        workSeq={item.data.WORK_SEQ}
        item={item}
      />
    );
