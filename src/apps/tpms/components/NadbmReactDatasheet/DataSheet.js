...
export default class DataSheet extends PureComponent {
  ...
  handleKey(e) {
    ...
    const equationKeysPressed = [187 /* equal */, 189 /* substract */, 190 /* period */, 107 /* add */, 109 /* decimal point */, 110].indexOf(keyCode) > -1;
  ...
  handleNavigate(e, offsets, jumpRow) {
    if (offsets && (offsets.i || offsets.j)) {
      ...
      const updateLocation = () => {
        ...
        // add plus new line event
        if (data[newLocation.i] === undefined) {
          if (data[newLocation.i - 1][0].readOnly !== true) {
            const newData = [...data[newLocation.i - 1]].map(() => ({
              value: '',
            }));
            data.push(newData);
            this.props.pushNewRow();
            this._setState({
              start: newLocation,
              end: newLocation,
              editing: {},
    ...
    if (isEditing) {
      ...
      if (currentCell && currentCell.component && !currentCell.forceComponent) {
        ...
        setTimeout(() => {
          ...
          // const index = this.state.end.i;
          // this.dgDom && this.dgDom.getElementsByClassName('data-grid')[0].firstElementChild.children[index].focus()
        }, 1);
      }
    }
  }
  ...
  onMouseDown(i, j) {
    const editing = isEmpty(this.state.editing) || this.state.editing.i !== i || this.state.editing.j !== j ? {} : this.state.editing;
    this._setState({
      selecting: true,
      start: { i, j },
      end: { i, j },
