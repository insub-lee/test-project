...
import Datasheet from '../NadbmReactDatasheet';
...
class ExcelViewer extends React.Component {
  ...
  render() {
    return (
      ...
      >
        <ContextMenu id="excelContextMenu">
          {this.state.selectedRow !== 0 && (
            <MenuItem data={{ rowIndex: this.state.selectedRow }} onClick={this.handleClick}>
              해당 Row 삭제
            </MenuItem>
          )}
        </ContextMenu>
        ...
        {this.state.grid.size > 0 && (
          <div style={styles.sheetOption}>
            <ContextMenuTrigger id="excelContextMenu">
              <Datasheet
                data={this.state.grid.toJS()}
                pushNewRow={this.handleAddNewRow}
                valueRenderer={cell => cell.value}
                // onContextMenu={(e, cell, i, j) => cell.readOnly ? e.preventDefault() : null}
                onContextMenu={this.handleContextMenuValue}
