...
function handler(apiOptions, actions) {
  ...
  const dialogFilesText = selectedResources.length > 1 ? `${selectedResources.length} ${getMessage('files')}` : `"${selectedResources[0].name}"`;
  ...
  return {
    ...
    shouldBeAvailable: apiOptions => {
      ...
      return selectedResources.every(resource => {
        ...
        if (apiOptions.canDelete) {
          switch (resource.type) {
            case 'file':
              return apiOptions.canDelete.file;
            case 'dir':
            default:
              return apiOptions.canDelete.dir;
          }
        }
        return false;
      });
