...
async function handler(apiOptions, actions) {
  ...
  const onStart = ({ archiveName, quantity }) => {
    ...
    const newChildren = notifUtils.addChild((notification && notification.children) || [], notificationChildId, childElement);
      ...
      return (
        ...
        selectedResources.every(resource => {
          if (resource.capabilities) {
            return resource.capabilities.canDownload;
          }
          return apiOptions.canDownload || false;
        })
      );
    },
