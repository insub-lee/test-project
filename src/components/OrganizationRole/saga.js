...
export function* getGroupData(payload) {
  const { ROLE_CD } = payload;
  const response = yield call(Axios.post, '/api/common/v1/account/getGroupData/', { ROLE_CD });
  const { groupData } = response;

  yield put({
    type: actionType.GET_GROUP_DATA,
    groupData,
  });
}

export function* getGroupMemberData(payload) {
  const { ROLE_CD, ACNT_TYPE, ACNT_ID } = payload;

  const response = yield call(Axios.post, '/api/common/v1/account/getGroupMemberData/', { ROLE_CD, ACNT_TYPE, ACNT_ID });
  const { groupMemberData } = response;

  yield put({
    type: actionType.GET_GROUP_MEMBER_DATA,
    groupMemberData,
  });
}

export function* getSearchResultData(payload) {
  const { ROLE_CD, ACNT_TYPE, ACNT_ID, KEYWORD } = payload;

  const response = yield call(Axios.post, '/api/common/v1/account/getSearchResultData/', { ROLE_CD, ACNT_TYPE, ACNT_ID, KEYWORD });
  const { searchResultData } = response;

  yield put({
    type: actionType.GET_SEARCH_RESULT_DATA,
    searchResultData,
  });
}
...
export default function* rootSaga() {
  ...
  yield takeLatest(actionType.GET_GROUP_DATA_SAGA, getGroupData);
  yield takeLatest(actionType.GET_GROUP_MEMBER_DATA_SAGA, getGroupMemberData);
  yield takeLatest(actionType.GET_SEARCH_RESULT_DATA_SAGA, getSearchResultData);
}
