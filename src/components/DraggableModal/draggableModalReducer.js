...
const getInitialModalState = (initialWidth, initialHeight) => {
  return {
    ...initialModalState,
    width: initialWidth,
    ...
    case 'drag':
      return {
        ...
        modals: {
          ...
          [action.id]: {
            ...state.modals[action.id],
            ...clampDrag(state.windowSize.width, state.windowSize.height, action.x, action.y, state.modals[action.id].width, state.modals[action.id].height),
            zIndex: getNextZIndex(state, action.id),
          },
        },
      };
