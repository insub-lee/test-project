...
class RootKeyCheckBoxComp extends PureComponent {
  constructor(props) {
    super(props);
    this.state = {
      categoryMapList: [],
  ...
  componentDidMount() {
    ...
    const { mapId, rootKey } = CONFIG.property;
    const apiArray = [{ key: `checkBoxData_${rootKey}`, url: `/api/admin/v1/common/categoryMapList`, type: 'POST', params: { PARAM: { NODE_ID: rootKey } } }];
  ...
  setCheckList = sagaKey => {
    ...
    const rootData = apiData && apiData.categoryMapList && apiData.categoryMapList.find(c => c.NODE_ID === rootKey);

    const categoryMapList =
      (apiData && apiData.categoryMapList && apiData.categoryMapList.filter(x => rootData && rootData.LVL + 1 === x.LVL && x.USE_YN === 'Y')) || [];
...
RootKeyCheckBoxComp.defaultProps = {
  readOnly: false,
...
export default RootKeyCheckBoxComp;
