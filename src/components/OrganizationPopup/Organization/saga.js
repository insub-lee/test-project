...
export function* loadingOrganizationUser(payload) {
  ...
  const organizationSearchResult = yield select(state => state.get('organizationPopup').get(organizationSearchResultNames[type]));
...
export function* loadingGridDataPstn(payload) {
  ...
  const pstnUsers = yield select(state => state.get('organizationPopup').get('pstnUsers'));
...
export function* loadingGridDataGrp(payload) {
  ...
  const grpUsers = yield select(state => state.get('organizationPopup').get('grpUsers'));
...
export function* loadingGridData(payload) {
  ...
  const users = yield select(state => state.get('organizationPopup').get('users'));
...
// ****************** 조직도 팝업용 ******************
export function* getFirstDeptUser(payload) {
  const { DEPT_ID } = payload;

  const response = yield call(Axios.post, '/api/common/v1/account/getFirstDeptUser', { DEPT_ID });

  if (response.USER_ID === -1) {
    yield put({
      type: actionType.NO_USER_ID,
    });
    return;
  }
...
export function* setLocale(payload) {
  ...
  yield put({ type: actionType.CHANGE_LOCALE, locale });
}
// ****************** 조직도 팝업용 끝 ******************
...
export default function* rootSaga() {
  ...
  yield takeLatest(actionType.GET_FIRST_DEPTUSER, getFirstDeptUser);
  yield takeLatest(actionType.GET_TREE_DATA_PROFILE_SAGA, getTreeDataForProfile);
  yield takeLatest(actionType.SET_LOCALE, setLocale);
}
