...
import SortableTree from './resource/react-sortable-tree';
          ...
          <div>
            <form style={{ display: 'block' }} onSubmit={event => event.preventDefault()}>
              <div className="searchOptions">
                ...
                {treeType === 'grp' && !siteId && !siteIdParam && (
            ...
            <SortableTree
              ...
              generateNodeProps={({ node }) => {
                ...
                return {
                  // 글자 배경색 주황색으로 표시
                  title: (
                    <button className={`${node.key === selectedIndex ? 'active' : ''}`} onClick={handleOnClick}>
...
Tree.propTypes = {
  ...
  selectedDept: PropTypes.object.isRequired, // 조직도 좌측 select에 선택되어 있는 부서 ID
...
// # 트리 위 selectbox의 option들의 value
// 구성원 탭: DEPT_ID
// 직위 탭: PSTN_ID
// 직책 탭: DUTY_ID
// 가상그룹 탭: SITE_ID ***
...
// selectedIndex: 현재 선택된 노드의 ID값, 주황색 배경의 네모로 표현
// searchString: 트리 상단 Input 태그의 검색어, 글자를 주황색으로 표현
